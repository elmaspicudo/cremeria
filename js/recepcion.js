...
function Guardar(tipo)
{
	if (frmCoti.getForm().isValid()) {
		...
		var valores="fecha="+fecha +"&ordenC="+folio +"&obs="+obs;
		...
		for(i=0;i<lista;i++)
		{
			var contenedord = document.getElementById("txtclave"+i);
				if(contenedord != null) {	
							...
							var umd=Ext.get('txtCosto'+i).getValue();
							valores+="&prod"+i+"="+prod +"&cant"+i+"="+cant+"&costo"+i+"="+costo+"&umd"+i+"="+umd;
				}			
		}
		var tipo=1;
		$.ajax({
	...
	var panel1 = new Ext.Panel({
			...
			region: 'center',
			width    : 650,
			 html:'<table cellspacing="0" cellpadding="0" id="mytable" style="width:98%"><thead><tr class="x-grid3-hd-row x-grid3-header" id="unt0">	<th style="width:10%; text-align: left; height:20px" >Codigo</th><th style="width: 10%; text-align: left;">	Unidades</th><th style="width: 10%; text-align: left;" >U. Medida</th><th style="width: 30%; text-align: left;">Nombre</th><th style="width: 10%; text-align: left;" >Costo</th><th style="width: 10%; text-align: left;" >Operación</th></tr></thead><tbody id="bdPro"></tbody><tfoot><tr id="trnew"><td style="text-align:left;" class="trinicio"><input type="text" id="txtCodigo" value="" class="sinborded"/><td style="text-align:left;" class="trinicio"><input type="text" id="txtUnidades" value="" class="sinborded"/></td><td class="trinicio" ><div id="tdumd"></div></td><td class="trinicio" ><div id="tdDes" ></div></td><td class="trinicio"></td><td class="trinicio"></td></tr><tr><td colspan="6" ><input type="button" onclick="ejecutarventa();" value="Cerrar Venta" /></td></tr></tfoot></table>'		
		});	
