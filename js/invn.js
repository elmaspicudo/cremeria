...
[{name: 'folio'},{name: 'folioFactura'},{name: 'responsable'},{name: 'fecha'},{name: 'operaciones'}]);	

var listaCot={};
    listaCot.datos=function()
                     {
                     listaCot.datos.superclass.constructor.call(this,
					 {proxy:new Ext.data.HttpProxy({url:'otroListado.php?listado=5',method:'POST'}),
					 remoteSort:true,
					 reader:jsonCot
					 });
...
Ext.extend(listaCot.datos,Ext.data.Store,{load_listaCot:function(tipo,valor,padre){this.load({params:{listado:5,cvepadre:padre,consulta:tipo,where:valor,start: 0, limit: 30}});}});
